...
from src.tools import Serializer
from src.tools import _serialize_sentence, _convert_tokens_into_index, _add_pos_seq, _handle_relation_data , _lm_serialize
import matplotlib.pyplot as plt
from src.utils import load_pkl, load_csv
...
def main(cfg):
    ...
    # 最好在 cpu 上预测
    cfg.use_gpu = False
    if cfg.use_gpu and oneflow.cuda.is_available():
        device = oneflow.device('cuda', cfg.gpu_id)
    else:
    ...
    x = dict()
    x['word'], x['lens'] = oneflow.tensor([data[0]['token2idx']]), oneflow.tensor([data[0]['seq_len']])
    
    if cfg.model_name != 'lm':
        x['head_pos'], x['tail_pos'] = oneflow.tensor([data[0]['head_pos']]), oneflow.tensor([data[0]['tail_pos']])
        if cfg.model_name == 'cnn':
            if cfg.use_pcnn:
                x['pcnn_mask'] = oneflow.tensor([data[0]['entities_pos']])
        if cfg.model_name == 'gcn':
            # 没找到合适的做 parsing tree 的工具，暂时随机初始化
            adj = oneflow.randint(0, 2, size=(1,data[0]['seq_len'],data[0]['seq_len']), dtype=oneflow.float)
            x['adj'] = adj
    ...
    with oneflow.no_grad():
        y_pred = model(x)
        y_pred = oneflow.softmax(y_pred, dim=-1)[0]
