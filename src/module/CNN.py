...
class CNN(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if self.pooling_strategy == 'max':
            ...
            x_len = mask.squeeze().eq(0).sum(-1).unsqueeze(-1).to(oneflow.float).to(device=mask.device)
            xp = oneflow.sum(x, dim=-1) / x_len
