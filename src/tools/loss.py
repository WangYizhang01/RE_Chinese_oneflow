...
class LabelSmoothSoftmaxCEV1(nn.Module):
    ...
    def forward(self, logits, label):
        ...
        with oneflow.no_grad():
            ...
            lb_one_hot = oneflow.empty_like(logits).fill_(
                lb_neg).scatter_(1, label.unsqueeze(1), lb_pos).detach()

        logs = self.log_softmax(logits)
        loss = -oneflow.sum(logs * lb_one_hot, dim=1)
        loss[ignore] = 0
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=0, alpha=None, size_average=True):
        ...
        if isinstance(alpha,(float,int)):
            self.alpha = oneflow.Tensor([alpha,1-alpha])
        if isinstance(alpha,list): 
            self.alpha = oneflow.Tensor(alpha)
    ...
    def forward(self, input, target):
        ...
        pt = oneflow.tensor(logpt.data.exp())
