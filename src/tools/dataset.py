...
def collate_fn(cfg):
    ...
    def collate_fn_intra(batch):
        ...
        x['word'] = oneflow.tensor(word)
        x['lens'] = oneflow.tensor(word_len)
        y = oneflow.tensor(y)

        if cfg.model_name != 'lm':
            x['head_pos'] = oneflow.tensor(head_pos)
            x['tail_pos'] = oneflow.tensor(tail_pos)
            if cfg.model_name == 'cnn' and cfg.use_pcnn:
                x['pcnn_mask'] = oneflow.tensor(pcnn_mask)
            if cfg.model_name == 'gcn':
                # 没找到合适的做 parsing tree 的工具，暂时随机初始化
                B, L = len(batch), max_len
                adj = oneflow.randint(0, 2, size=(B, L, L), dtype=oneflow.float)
                x['adj'] = adj
        return x, y
